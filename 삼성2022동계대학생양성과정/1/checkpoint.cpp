...
void cancel(int mNumber) //주문 취소
{
    ...
    for (idx = 0; idx < order_list[mStock][mBuy].size(); idx++)
    {
        if (order_list[mStock][mBuy][idx].mNumber == mNumber) //완전 탐색으로 mNumber번 주문 탐색 후 가격과 주문 수량을 0으로 처리하여 주문 취소
        {
            order_list[mStock][mBuy].erase(order_list[mStock][mBuy].begin() + idx);
            return;
        }
    }
}
